...
# conductor power, multi-band harmony, and phase-slip dynamics in source space.
...
class EEGSourceReconstructionApp:
    ...
    # --- NEW --- UI for the Coordination Model
    def create_coordination_tab(self):
        ...
        self.coord_viz_var = tk.StringVar(value="Phase-Slip Rate")
        metric_options = [
    ...
    # --- Most of the backend logic from here on is consolidated ---
    # --- (load_file, update_status, run_reconstruction, etc.) ---
    ...
    def _process_data(self):
        ...
        try:
            ...
            # --- NEW: Diverging logic based on analysis mode ---
            ...
            if mode == "standard":
                ...
                # Note: We use the already preprocessed (but not band-filtered) data
    ...
    # --- NEW: The core function for the Universal Coordination Model ---

    def _analyze_coordination_in_source_space(self, stc_broadband):
        """
        Calculates the Universal Brain Coordination metrics on the source-level data, 
        preserving the time dimension for animation.
        """
        ...
        orchestra_freqs = []
        for band, var in self.orchestra_vars.items():
            if var.get() and band != conductor_band_name:
                orchestra_freqs.append(self.freq_bands[band])

        if not orchestra_freqs:
            raise ValueError("The 'Orchestra' must contain bands different from the 'Conductor'.")

        min_orch_freq = min(f[0] for f in orchestra_freqs)
        max_orch_freq = max(f[1] for f in orchestra_freqs)

        # Filter the source-space data to get the conductor and moiré signals
        ...
        # X-Axis: Conductor Power (Dominance)
        x_data = stc_conductor.data ** 2
        stc_x = mne.SourceEstimate(x_data, vertices=stc_broadband.vertices, tmin=stc_broadband.tmin, tstep=stc_broadband.tstep, subject='fsaverage')

        # Y-Axis: Moiré Power (Harmony)
        y_data = stc_moire.data ** 2
        stc_y = mne.SourceEstimate(y_data, vertices=stc_broadband.vertices, tmin=stc_broadband.tmin, tstep=stc_broadband.tstep, subject='fsaverage')
        
        # Z-Axis: Phase-Slip Rate (Coordination)
        phase_conductor = np.angle(hilbert(stc_conductor.data, axis=1))
        phase_moire = np.angle(hilbert(stc_moire.data, axis=1))
        phase_diff = phase_conductor - phase_moire
        
        # We now keep the time dimension instead of averaging
        plv_instantaneous = np.abs(np.exp(1j * phase_diff))
        z_data = 1 - plv_instantaneous  # High value = low locking = "slip"
        stc_z = mne.SourceEstimate(z_data, vertices=stc_broadband.vertices, tmin=stc_broadband.tmin, tstep=stc_broadband.tstep, subject='fsaverage')

        self.log_result("✓ Time-resolved coordination analysis complete.")
        return {'Conductor Power': stc_x, 'Moiré Harmony': stc_y, 'Phase-Slip Rate': stc_z}
    ...
    # --- NEW: Visualization logic for the Coordination Model ---
    ...
    def _process_coordination_visualization(self, coord_metrics, metric_to_show):
        ...
        if metric_to_show == 'Conductor Power' or metric_to_show == 'Moiré Harmony':
            ...
            # This maps the 1st, 50th, and 99th percentile of your data to the
            # bottom, middle, and top of the color bar.
            params = {
                'colormap': 'viridis', 
